...
import {OSS_DOMAIN} from "@/utils/constants";
...
const Step1: React.FC<Step1Props> = forwardRef(({articleId}, ref) => {
  ...
  const handleChange: UploadProps['onChange'] = ({ fileList: newFileList }) => {
    //真实的地址是`http://image.localhost/${file.response.key}`
      ...
      form.validateFields()
        .then(async (values) => {
          ...
          const params: Article.ArticleCreate = {
            ...values,
            tags: selectedIds,
            thumbnails: fileList.map(file => `http://${OSS_DOMAIN}/${file.response.key}`)
          }
      ...
      form.validateFields()
        .then(async (values) => {
          ...
          const params: Article.ArticleBasicUpdate = {
            ...values,
            tags: selectedIds,
            thumbnails: fileList.map(file => file.url || `http://${OSS_DOMAIN}/${file.response.key}`)
          }
